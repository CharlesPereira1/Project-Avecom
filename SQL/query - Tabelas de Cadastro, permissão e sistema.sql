...
/*O PROCESSO DE CADASTRO E PERMISSÕES NO SISTEMA SÃO DIVIDIDOS EM:
	...
	2 - TELA      - SELECT * FROM UCA.cafstel0
	3 - OBJETO    - SELECT * FROM uca.cafsobj0
	4 - PERMISSÃO - SELECT * FROM uca.cafsper0
*/

/*COPIAR PERMISSÃO DE TELA DE USUÁRIO PARA USUÁRIO*/

select max(cod_tela) from UCA.cafsper0 where  cod_sistema='IA2' --and cod_tela=33
select * from UCA.cafsper0 where  cod_sistema='IA2'
...
--Verificar qual é o ultimo cód de tela para inserir na tag do component da unit e gerar o insert
select max(cod_tela) from UCA.cafstel0 WHERE cod_sistema='IA2'
	--INSERT INTO uca.cafstel0 (cod_tela, cod_sistema, nom_tela)
	--LUES (98, 'IA2', 'Relat -> 10.1 - Relatório de Ovoscopia')
	--GO

--Adicionar o Objeto copiando de um que já está criado
select * from uca.cafsobj0 WHERE cod_sistema='IA2'and cod_tela=97
	--INSERT INTO uca.cafsobj0 (cod_objeto, cod_tela, cod_sistema, nom_objeto)
	--VALUES (6, 98, 'IA2', 'Visualizar Relatório')
	--GO

--Adicionar permissão aos usuários que ainda não tenham
SELECT usuario, 6 cod_objeto, 98 cod_tela, cod_sistema from UCA.cafsper0 where cod_sistema='IA2' and cod_tela=33 AND cod_objeto=1
	--INSERT INTO uca.cafsper0 (usuario, cod_objeto, cod_tela, cod_sistema)
	--VALUES ('Admin', 6, 98, 'IA2')
	--GO
